...
def dfs(directory, output, folder_indexes,depth=1,maxDepth=True):
    try:
        ignore_list = ('.git', 'README.md', 'path.md')
        ...
        for idx,item in enumerate(items,start=1):
            ...
            if os.path.isdir(item_path):
                ...
                print(f"{space}- {'#' * min(depth, 6)} 第{'-'.join(folder_indexes)}章 [{item}]({process_path(item_path,folder_name)})",file=output)
                ...
                print(f"{space}- {title} [{item}]({process_path(item_path,folder_name)})",file=output)
